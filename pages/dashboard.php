...
<body>
        ...
        <div class="container-fluid px-4 px-lg-5">
            ...
            <div class="collapse navbar-collapse justify-content-center" id="navbarNav">
                <div class="nav-pill-container mt-3 mt-lg-0">
                    ...
                    <a class="nav-link text-danger d-lg-none" href="#" onclick="PARFY.auth.logout()">LOGOUT</a>
                </div>
                <!-- Mobile Search Input -->
            ...
            <div class="topbar d-none d-lg-flex align-items-center">
                <div class="search-box-wrapper me-3" id="searchBoxWrapper" style="position: relative;">
                    <div class="search-input-group" id="searchInputGroup">
                        <i class="bi bi-search text-white-50 me-2" style="font-size: 0.9rem;"></i>
                        <input type="text" id="navbarSearchInput" placeholder="Cari parfum, brand, aroma..." oninput="handleSearch(this.value)" onkeyup="handleSearchKey(event)">
                        <i class="bi bi-x-lg text-white-50" style="cursor: pointer; font-size: 0.8rem; margin-left: 8px;" onclick="closeSearch()"></i>
                    </div>
                    <i class="bi bi-search" id="searchToggleBtn" title="Cari Parfum" onclick="toggleSearch()" style="cursor:pointer; font-size: 1.15rem;"></i>
    ...
    <script>
            ...
            <div class="col-6 col-md-3">
                <div class="product-card" style="cursor: pointer;" onclick="window.location.href='/coding web IMK/parfy-php/detail-produk?id=${product.id}'">
                    ...
                    <div class="card-footer-custom">
                        <a href="#" class="btn-buy-now" onclick="event.stopPropagation(); buyNow('${product.id}')">
        ...
        function redirectToLogin() {
            Swal.fire('Informasi', 'Silakan login terlebih dahulu untuk melihat detail produk atau berbelanja!', 'info');
    ...
    <div id="cs-widget-container"
        ...
        <!-- Chat Button (FAB) -->
        <button id="cs-fab" onclick="toggleChat()" style="
    ...
    <script>
        ...
        async function sendMessage() {
            ...
            try {
                const user = typeof PARFY !== 'undefined' ? PARFY.getUser() : null;
                const userName = user && user.name ? user.name : '';
                const context = chatHistory.slice(-5);
                const res = await fetch('/coding web IMK/parfy-php/api/chat/index.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ message: message, history: context, userName: userName })
                });
                ...
                if (data.reply) {
                    ...
                    addMessage('bot', 'Maaf, saya sedang mengalami gangguan sementara.');
                }
            } catch (error) {
                removeMessage(typingId);
        ...
        function formatChatText(text) {
            ...
            // Convert markdown italic _text_ to <em>$1</em>
            formatted = formatted.replace(/_(.*?)_/g, '<em>$1</em>');
            // Convert markdown links [title](url) to clickable styled links
            formatted = formatted.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" style="color: #005c97; font-weight: 600; text-decoration: underline;">$1</a>');
        ...
        function addMessage(role, text) {
            ...
            msgDiv.innerHTML = `<div>${formatChatText(text)}</div>`;
